...
//#define LCD_RST_GPIO_CLK_ENABLE()       do{ RCC->APB2ENR |= 1 << x; }while(0)   /* 所在IO口时钟使能 */

#define LCD_WR_GPIO_PORT                GPIOD
#define LCD_WR_GPIO_PIN                 SYS_GPIO_PIN5
#define LCD_WR_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 5; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RD_GPIO_PORT                GPIOD
#define LCD_RD_GPIO_PIN                 SYS_GPIO_PIN4
#define LCD_RD_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 5; }while(0)   /* 所在IO口时钟使能 */

#define LCD_BL_GPIO_PORT                GPIOB
#define LCD_BL_GPIO_PIN                 SYS_GPIO_PIN0
#define LCD_BL_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

/* LCD_CS(需要根据LCD_FSMC_NEX设置正确的IO口) 和 LCD_RS(需要根据LCD_FSMC_AX设置正确的IO口) 引脚 定义 */
#define LCD_CS_GPIO_PORT                GPIOG
#define LCD_CS_GPIO_PIN                 SYS_GPIO_PIN12
#define LCD_CS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RS_GPIO_PORT                GPIOG
#define LCD_RS_GPIO_PIN                 SYS_GPIO_PIN0
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */
