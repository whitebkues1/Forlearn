...
#define OV7725_VSYNC_GPIO_CLK_ENABLE()  do{ RCC->APB2ENR |= 1 << 2; }while(0)   /* OV7725帧同步脚, 所在IO口时钟使能 */

#define OV7725_WRST_GPIO_PORT           GPIOD
#define OV7725_WRST_GPIO_PIN            SYS_GPIO_PIN6
#define OV7725_WRST_GPIO_CLK_ENABLE()   do{ RCC->APB2ENR |= 1 << 5; }while(0)   /* FIFO写指针复位脚, 所在IO口时钟使能 */

#define OV7725_RRST_GPIO_PORT           GPIOG
#define OV7725_RRST_GPIO_PIN            SYS_GPIO_PIN14
#define OV7725_RRST_GPIO_CLK_ENABLE()   do{ RCC->APB2ENR |= 1 << 8; }while(0)   /* FIFO读指针复位脚, 所在IO口时钟使能 */

#define OV7725_OE_GPIO_PORT             GPIOG
#define OV7725_OE_GPIO_PIN              SYS_GPIO_PIN15
#define OV7725_OE_GPIO_CLK_ENABLE()     do{ RCC->APB2ENR |= 1 << 8; }while(0)   /* FIFO数据输出使能脚, 所在IO口时钟使能 */

#define OV7725_RCLK_GPIO_PORT           GPIOB
#define OV7725_RCLK_GPIO_PIN            SYS_GPIO_PIN4
#define OV7725_RCLK_GPIO_CLK_ENABLE()   do{ RCC->APB2ENR |= 1 << 3; }while(0)   /* FIFO读数据时钟脚, 所在IO口时钟使能 */

#define OV7725_WEN_GPIO_PORT            GPIOB
#define OV7725_WEN_GPIO_PIN             SYS_GPIO_PIN3
#define OV7725_WEN_GPIO_CLK_ENABLE()    do{ RCC->APB2ENR |= 1 << 3; }while(0)   /* FIFO写数据使能脚, 所在IO口时钟使能 */
