...
void norflash_init(void)
{
    ...
    sys_gpio_set(NORFLASH_CS_GPIO_PORT, NORFLASH_CS_GPIO_PIN,
...
#ifdef SRAMIN
    norflash_buf = mymalloc(SRAMIN, 4096);  /* 申请内存 */
    if(norflash_buf == NULL)
    {
        return ;                            /* 申请失败 */
    }
#else
    norflash_buf = g_norflash_buf;          /* 直接指向全局数组 */
#endif
...
#ifdef SRAMIN
    myfree(SRAMIN, norflash_buf);   /* 释放内存 */
#endif
