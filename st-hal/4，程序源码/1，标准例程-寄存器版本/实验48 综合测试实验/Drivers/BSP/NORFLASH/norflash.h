 ...
 *
 * V1.01 20221026
 * 1，将g_norflash_buf改成内存管理形式
 * 2，norflash_write函数,添加OS相关限制,避免写入出错
 *
 ****************************************************************************************************
 */
...
#define NORFLASH_CS_GPIO_PIN            SYS_GPIO_PIN12
#define NORFLASH_CS_GPIO_CLK_ENABLE()   do{ RCC->APB2ENR |= 1 << 3; }while(0)   /* PB口时钟使能 */

/******************************************************************************************/

/* NORFLASH 片选信号 */
#define NORFLASH_CS(x)      sys_gpio_pin_set(NORFLASH_CS_GPIO_PORT, NORFLASH_CS_GPIO_PIN, x)
