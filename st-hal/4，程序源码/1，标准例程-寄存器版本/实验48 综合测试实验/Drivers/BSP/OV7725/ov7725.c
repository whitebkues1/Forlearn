...
uint8_t ov7725_read_reg(uint16_t reg)
{
    uint8_t data = 0;
    sccb_start();                   /* 启动SCCB传输 */
    sccb_send_byte(OV7725_ADDR);    /* 写器件ID */
    sccb_send_byte(reg);            /* 写寄存器地址 */
    sccb_stop();
    
    /* 设置寄存器地址后，才是读 */
    sccb_start();
    sccb_send_byte(OV7725_ADDR | 0X01); /* 发送读命令 */
...
uint8_t ov7725_write_reg(uint8_t reg, uint8_t data)
{
    ...
    if (sccb_send_byte(OV7725_ADDR))res = 1;/* 写器件ID */
...
uint8_t ov7725_init(void)
{
    ...
    RCC->APB2ENR |= 1 << 4;         /* C0~C7引脚  GPIO时钟使能 */

    sys_gpio_set(OV7725_VSYNC_GPIO_PORT, OV7725_VSYNC_GPIO_PIN,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* VSYNC 引脚模式设置 */

    sys_gpio_set(OV7725_WRST_GPIO_PORT, OV7725_WRST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* WRST  引脚模式设置 */

    sys_gpio_set(OV7725_RRST_GPIO_PORT, OV7725_RRST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* RRST  引脚模式设置 */

    sys_gpio_set(OV7725_OE_GPIO_PORT, OV7725_OE_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* OE    引脚模式设置 */

    sys_gpio_set(OV7725_RCLK_GPIO_PORT, OV7725_RCLK_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* RCLK  引脚模式设置 */

    sys_gpio_set(OV7725_WEN_GPIO_PORT, OV7725_WEN_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* WEN   引脚模式设置 */

    sys_gpio_set(GPIOC, 0XFF,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* D0~D7 引脚模式设置 */
    ...
    sys_gpio_remap_set(24, 3, 2);   /* 关闭JTAG, 使能SWD, 这样 PB3, PB4, PA15, 才可以当普通IO使用 */
