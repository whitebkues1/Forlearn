...
#define SRAM_WR_GPIO_CLK_ENABLE()       do{ RCC->APB2ENR |= 1 << 5; }while(0)   /* 所在IO口时钟使能 */

#define SRAM_RD_GPIO_PORT               GPIOD
#define SRAM_RD_GPIO_PIN                SYS_GPIO_PIN4
#define SRAM_RD_GPIO_CLK_ENABLE()       do{ RCC->APB2ENR |= 1 << 5; }while(0)   /* 所在IO口时钟使能 */

/* SRAM_CS(需要根据SRAM_FSMC_NEX设置正确的IO口) 引脚 定义 */
#define SRAM_CS_GPIO_PORT                GPIOG
#define SRAM_CS_GPIO_PIN                 SYS_GPIO_PIN10
#define SRAM_CS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 8; }while(0)   /* 所在IO口时钟使能 */
