    ...
    if ((type & 0X0f) >= BTN_TYPE_ANG)  /* 边角按钮/文字按钮 */
    {
        switch (type & 0X0f)
        {
            case BTN_TYPE_ANG:/* 边角按钮 */
                btn_crt->bkctbl = (uint16_t *)gui_memin_malloc(4 * 2); /* 分配8个字节 */
            ...
            case BTN_TYPE_TEXTA:/* 背景透明的文字按钮 */
                btn_crt->bkctbl = (uint16_t *)gui_memin_malloc(width * height * 2); /* 要分配完整的背景色表 */
            ...
            case BTN_TYPE_TEXTB:/* 背景单一的文字按钮 */
                btn_crt->bkctbl = (uint16_t *)gui_memin_malloc(2 * 2); /* 分配4个字节 */
 ...
 * @param       btnx            : 按钮指针
 * @param       in_key          : 输入按键指针
 * @retval      0, 没有有效操作; 1, 有一次有效操作;
 */
uint8_t btn_check(_btn_obj *btnx, void *in_key)
{
...
/**
 * @brief       画图片按钮
 *  @note       图片按钮,如果不想被填充,请设置背景色为0X0001,否则每次都会先被填充后画图
 * @param       btnx            : 按钮指针
 * @retval      无
 */
...
{
    uint16_t angcolor = 0, upcolor = 0, downcolor = 0;
    uint8_t sta;
