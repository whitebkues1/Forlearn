 ...
 *              [1]:0,不显示光标;             1,显示光标;
 *              [0]:0,标准edit,满之不能再写入;1,自动清除edit,满之后自动清除,回到原位;
 * @param       font            : 编辑框字体
 * @retval      编辑框指针(0,表示创建失败)
 */
 ...
 * @retval      无
 */
void edit_cursor_set(_edit_obj *editx, uint16_t x)
{
    uint16_t newpos;
    uint16_t maxpos = strlen((const char *)editx->text);
...
#define	EDIT_LR_EXTEND      20      /* 左右扩展范围 */
    ...
    uint16_t ttop, tleft;
    static uint8_t chooseflag = 0;  /* 选中标记 */
