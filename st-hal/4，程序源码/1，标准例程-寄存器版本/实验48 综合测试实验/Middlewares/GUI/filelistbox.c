 ...
 * V1.2 20160528
 * 根据最新fatfs(0.12版本),修改文件索引保存方式,采用dptr方式.
 * V1.3 20160901
 * 1,将所有内存申请都放到内部SRAM!!
 * 2,修正filelistbox_get_pathdepth函数的bug,添加对参数地址为0时的判断.
 * V1.4 20220526
 * 1, 修改注释方式
 ...
 *  @note                         height高度必须为:gui_phy.listheight*N+gui_phy.listheight+gui_phy.listheight,这里默认为40+20的N倍
 *                                width:宽度必须大于等于gui_phy.listheight(20)
 *
 * @param       type            : bit7,用于是否显示滚动条(自动控制),其他位保留.
 * @param       font            : 字体大小
 * @retval      filelistbox指针(0,表示创建失败)
 */
_filelistbox_obj *filelistbox_creat(uint16_t left, uint16_t top, uint16_t width, uint16_t height, uint8_t type, uint8_t font)
{
    ...
    gui_memset((uint8_t *)filelistbox_crt, 0, sizeof(_filelistbox_obj));    /* 将filelistbox_crt的值全部设置为0 */
...
/**
 * @brief       查找编号为index的flist
 * @param       flistx          : 当前的flist指针
 * @param       index           : 要寻找的flist编号 0~totalitems-1;
 * @retval      filelistbox指针(0,表示查找失败)
 */
_filelistbox_list *filelist_search(_filelistbox_list *filelistx, uint16_t index)
{
    uint16_t icnt = 0;
...
/**
 * @brief       删除所有链表
 * @param       filelistdel     : 要删除的链表中的某个成员
 * @retval      无
 */
...
/**
 * @brief       删除filelistbox
 * @param       filelistbox_del : 要删除的filelistbox
 * @retval      无
 */
...
/**
 * @brief       重建文件列表框
 *  @note       当有效文件使用完退出后使用,重新扫描文件，并重画
 * @param       filelistbox     : filelistbox指针
 * @retval      0, 成功; 其他, 失败;
 */
uint8_t filelistbox_rebuild_filelist(_filelistbox_obj *filelistbox)
{
    uint8_t res = 0;
 ...
 * @retval      未用到
 */
uint8_t filelistbox_check(_filelistbox_obj *filelistbox, void *in_key)
{
    _in_obj *key = (_in_obj *)in_key;
    _filelistbox_list *filelistx;
    uint16_t endx;/* 条目显示区的尾端 */
    uint16_t tempindex;
    uint16_t i;
    uint16_t lastindex;
    static uint8_t scrollbarflag = 0;   /* scrollbar标记 */
...
/**
 * @brief       增加一条filelistbox的条目
 * @param       filelistbox     : filelistbox指针
 * @param       name            : 条目名字
 * @param       type            : 类型
 * @retval      0, 成功;  其他, 失败;
 */
uint8_t filelistbox_addlist(_filelistbox_obj *filelistbox, uint8_t *name, uint8_t type)
{
    _filelistbox_list *filelistx;
    _filelistbox_list *filelisttemp;
    ...
    filelistx->name = (uint8_t *)strcpy((char *)filelistx->name, (const char *)name);   /* 复制name的内容到 */
...
void filelistbox_show_path(_filelistbox_obj *filelistbox)
{
    ...
    if (plen)   /* 需要显示 */
    {
        ...
        if (plen * (gui_phy.listfsize / 2) > (filelistbox->width - filelistbox->icosize - 7))
        {
            ...
            {
                path = (uint8_t *)strstr((const char *)++path, (const char *)chgchar);  /* 去掉一截 */
...
/* 磁盘容量字符串 */
uint8_t *const FLBOX_DISK_SIZE_CAPTION_TBL[GUI_LANGUAGE_NUM] =
{
    "磁盘容量:", "磁盤容量:", "Capacity:",
};

/* 磁盘可用空间字符串 */
uint8_t *const FLBOX_DISK_FREE_CAPTION_TBL[GUI_LANGUAGE_NUM] =
{
    "可用空间:", "可用空間:", "Free:",
};

/* 文件夹字符串 */
uint8_t *const FLBOX_FOLDER_CAPTION_TBL[GUI_LANGUAGE_NUM] =
{
    "文件夹:", "文件夾:", "Folder:",
};

/* 磁盘可用空间字符串 */
uint8_t *const FLBOX_FILES_CAPTION_TBL[GUI_LANGUAGE_NUM] =
{
    "  文件:", "  文件:", "  Files:",
};

/* 磁盘名字表 */
uint8_t *const FLIST_DISK_NAME_TBL[FF_VOLUMES][GUI_LANGUAGE_NUM] =
{
    {"SD卡", "SD卡", "SD Card",},
    {"SPI FLASH磁盘", "SPI FLASH磁盤", "SPI FLASH Disk",},
    {"U盘", "U盤", "U Disk",},
};

/* filelist磁盘顺序表,即选择编号与磁盘编号对应关系 */
volatile uint8_t FLIST_DISK_ORDER_TBL[FF_VOLUMES];

/* filelist磁盘路径表 */
uint8_t *const FLIST_DISK_PATH_TBL[FF_VOLUMES] = {"0:", "1:", "2:"};
...
void filelistbox_show_info(_filelistbox_obj *filelistbox)
{
    ...
    FATFS *fs1;
    uint8_t res;
    uint32_t freclust = 0, fresec = 0, totsec = 0;
    uint8_t *temp;
    uint8_t *temp1;
    uint8_t *mark;
    ...
    {
        mark = (uint8_t *)FLIST_DISK_PATH_TBL[FLIST_DISK_ORDER_TBL[filelistbox->selindex]];
...
#if FF_MAX_SS!=512                          /* 对于扇区区不是512字节大小的磁盘 */
...
uint8_t filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
    ...
    uint16_t typeex = 0;
    uint32_t fileoffset;
    uint8_t mark = 0; /* 0,查找文件夹;1,查找目标文件;2 */
    flboxfinfo = (FILINFO *)gui_memin_malloc(sizeof(FILINFO)); /* 先申请内存 */
    ...
    if (filelistbox->path)  /* 路径有效 */
    {
        ...
        if (res == FR_OK)  	/* 打开目录成功了 */
        {
            ...
            filelistbox->findextbl = (uint32_t *)gui_memin_malloc(FLBOX_MAX_DFFILE * 4); /* 先默认申请FLBOX_MAX_DFFILE个目标文件索引内存 */
        }

        while (res == FR_OK)
        {
            fileoffset = flboxdir.dptr; /* 记录下此文件的dptr值 */
            ...
            {
                type = exfuns_file_type((char *)(flboxfinfo->fname));    /* 获得类型 */
                ...
                if (((1 << type)&filelistbox->fliter) && ((1 << typeex)&filelistbox->fliterex))   /* 大小类型都正确 */
                {
                    ...
                    if (filelistbox->filecnt < FLBOX_MAX_DFFILE)filelistbox->findextbl[filelistbox->filecnt] = fileoffset; /* 记录下文件dptr索引 */
            ...
            if (filelistbox_addlist(filelistbox, (uint8_t *)flboxfinfo->fname, type))break; /* 添加失败了 */
        }

        if (mark == 1 && filelistbox->filecnt)
        {
            fn = gui_memin_realloc(filelistbox->findextbl, filelistbox->filecnt * 4); /* 释放多余的内存 */

            if (fn != NULL)filelistbox->findextbl = (uint32_t *)fn; /* 如果申请成功了,则使用新的地址,否则源地址不变 */
        }
    }
    ...
    gui_memin_free(flboxfinfo);	/* 释放内存 */
    return res;
}

/**
 * @brief       filelist添加磁盘(在filelist启动前,必须执行一次该函数)
 * @param       filelistbox     : filelistbox指针
 * @retval      无
 */
void filelistbox_add_disk(_filelistbox_obj *filelistbox)
{
    ...
    if (filelistbox->memdevflag & (1 << 0)) /* SD卡在位 */
    {
        ...
        {
            filelistbox_addlist(filelistbox, (uint8_t *)FLIST_DISK_NAME_TBL[0][gui_phy.language], 0); /* 添加一个磁盘 */
        }
    }

    if (filelistbox->memdevflag & (1 << 1)) /* SPI FLASH盘在位 */
    {
        ...
        {
            filelistbox_addlist(filelistbox, (uint8_t *)FLIST_DISK_NAME_TBL[1][gui_phy.language], 0); /* 添加一个磁盘 */
        }
    }

    if (filelistbox->memdevflag & (1 << 2))  	/* NAND FLASH盘在位 */
    {
        ...
        {
            filelistbox_addlist(filelistbox, (uint8_t *)FLIST_DISK_NAME_TBL[2][gui_phy.language], 0); /* 添加一个磁盘 */
        }
    }

    if (filelistbox->memdevflag & (1 << 3)) /* U盘在位 */
    {
        FLIST_DISK_ORDER_TBL[i++] = 3;
        res = f_getlabel ((TCHAR *)"3:", (TCHAR *)tbuf, (DWORD *)&sn);
        ...
        {
            filelistbox_addlist(filelistbox, (uint8_t *)FLIST_DISK_NAME_TBL[3][gui_phy.language], 0); /* 添加一个磁盘 */
        }
    }
}
 ...
 * @retval      路径深度(文件夹深度)
 */
uint8_t filelistbox_get_pathdepth(uint8_t *path)
{
    uint8_t depth = 0;
...
void filelistbox_check_filelist(_filelistbox_obj *filelistbox)
{
    ...
    if (filelistx->type == 0 || filelistx->type == 1)   /* 属于磁盘/文件夹,则打开 */
    {
        ...
        {
            str = (uint8_t *)FLIST_DISK_PATH_TBL[FLIST_DISK_ORDER_TBL[filelistbox->selindex]];
            gui_memin_free(filelistbox->path);/* 释放之前申请的内存 */
            filelistbox->path = (uint8_t *)gui_memin_malloc(strlen((const char *)str) + 1); /* 重新申请内存 */
        ...
        else     /* 是文件夹 */
        {
            if (strcmp((const char *)filelistx->name, ".."))   /* 不是..文件名 */
            {
                ...
                oldpath = (uint8_t *)gui_memin_malloc(strlen((const char *)filelistbox->path) + 1); /* 申请内存 */

                if (oldpath == NULL)return; /* 申请内存失败 */

                oldpath = (uint8_t *)strcpy((char *)oldpath, (const char *)filelistbox->path); /* 拷贝filelistbox->path到oldpath里面 */
                gui_memin_free(filelistbox->path);/* 释放filelistbox->path之前申请的内存 */
                filelistbox->path = (uint8_t *)gui_memin_malloc(ttlen + 2); /* 重新申请内存 */
            ...
            {
                filelistbox_back(filelistbox);/* 返回上一层 */
                return ;
            }
        }
...
/**
 * @brief       向上一级操作.(返回操作)
 *  @note       此函数可以单独调用,自动返回上一层.自带了扫描及重画功能.
 * @param       filelistbox     : filelistbox指针
 * @retval      无
 */
...
/* 创建 */
void tsflistbox_creat(uint16_t x, uint16_t y, uint16_t lenth, uint16_t height, uint8_t type, uint8_t fsize)
{
    tfilelistbox = filelistbox_creat(x, y, lenth, height, type, fsize);
}

/* 设置路径 */
void tsflistbox_setpath(uint8_t *path)
{
    gui_memin_free(tfilelistbox->path);/* 释放之前的 */
    tfilelistbox->path = (uint8_t *)gui_memin_malloc(strlen((const char *)path) + 1); /* 分配内存 */

    if (tfilelistbox->path == NULL)return  ; /* 内存分配不够 */

    tfilelistbox->path = (uint8_t *)strcpy((char *)tfilelistbox->path, (const char *)path);
}
...
/* 添加1个listbox条目 */
uint16_t tsflistbox_addlist(uint8_t *name, uint8_t type)
{
