 ...
 *  @note                         width的范围:必须大于等于LBOX_ITEM_HEIGHT
 *                                height的范围:必须是LBOX_ITEM_HEIGHT的整数倍.
 *
 * @param       type            : bit7,用于是否显示滚动条(自动控制),其他位保留.
 * @param       font            : 字体大小
 * @retval      listbox指针(0,表示创建失败)
 */
_listbox_obj *listbox_creat(uint16_t left, uint16_t top, uint16_t width, uint16_t height, uint8_t type, uint8_t font)
{
...
/**
 * @brief       查找编号为index的list
 * @param       listx           : 当前的list指针
 * @param       index           : 要寻找的list编号 0~totalitems-1;
 * @retval      listbox指针(0,表示查找失败)
 */
_listbox_list *list_search(_listbox_list *listx, uint16_t index)
{
    uint16_t icnt = 0;
...
/**
 * @brief       删除listbox
 * @param       listbox_del     : 要删除的链表中的某个成员
 * @retval      无
 */
...
/**
 * @brief       检查listbox
 * @param       listbox         : listbox指针
 * @param       in_key          : 输入按键指针
 * @retval      未用到
 */
uint8_t listbox_check(_listbox_obj *listbox, void *in_key)
{
...
/**
 * @brief       增加一条listbox的条目
 * @param       listbox         : listbox指针
 * @param       name            : 条目名字
 * @retval      0, 成功; 1, 失败;
 */
uint8_t listbox_addlist(_listbox_obj *listbox, uint8_t *name)
{
    _listbox_list *listx;
    _listbox_list *listtemp;
