...
_window_obj *window_creat(uint16_t left, uint16_t top, uint16_t width, uint16_t height, uint8_t id, uint8_t type, uint8_t font)
{
    ...
    if (type & (1 << 6)) /* 需要读取背景色 */
    {
        window_crt->bkctbl = (uint16_t *)gui_memex_malloc(width * height * 2); /* 要分配完整的背景色表 */
...
{
    uint8_t winclosebtnsize = 0;    /* 关闭按钮的尺寸 */
    ...
    if (windowx->type & (1 << 0))   /* 圆角窗,仿android的时候,不支持按钮(放了难看) */
    {
        ...
        gui_draw_arcrectangle(windowx->left, windowx->top, windowx->width, windowx->height, windowx->arcwinr, 0, ARC_WIN_RIM_COLOR, ARC_WIN_RIM_COLOR); /* 画出标题栏 */
    }
    else    /* 方角窗 */
    {
        /* 填充内部 */
...
/**
 * @brief       在制定位置显示一个msg box
 * @param       x,y             : 起始坐标
 * @param       width,height    : 宽度和高度
 * @param       str             : 字符串
 * @param       caption         : 消息窗口名字
 * @param       font            : 字体大小
 * @param       color           : 颜色
 * @param       mode            : 模式
 *              [7]:0,没有关闭按钮.1,有关闭按钮
 ...
 * @param       time            : 延时时间,单位:ms(仅在没有按键且需要读取背景色的时候,有效,最大65535)
 * @retval      
 *              0,没有任何按键按下/产生了错误.
 *              1,确认按钮按下了.
 *              2,取消按钮按下了.
...
uint8_t window_msg_box(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t *str, uint8_t *caption, uint8_t font, uint16_t color, uint8_t mode, uint16_t time)
{
    ...
    uint16_t temp, strheight = 0;

    _window_obj *twin = 0;      /* 窗体 */
    _btn_obj *okbtn = 0;        /* 确定按钮 */
    _btn_obj *cancelbtn = 0;    /* 取消按钮 */

    if (width < MSG_BOX_BTN2_WIDTH || height < (WIN_CAPTION_HEIGHT + font + 5))return 0; /* 尺寸错误 */
    ...
    if (mode & (1 << 1)) /* 需要显示取消按键 */
    {
        ...
        {
            cancelbtn->caption = (uint8_t *)GUI_CANCEL_CAPTION_TBL[gui_phy.language]; /* 确认 */
