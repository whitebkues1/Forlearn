...
extern uint16_t sms_yoff;           //显示在y轴方向的偏移量
extern uint16_t sms_xoff;           //显示在x轴方向的偏移量(实际显示宽度=256-2*sms_xoff)
extern uint32_t sms_rgb_parm1;
extern uint16_t sms_rgb_parm2;
extern uint16_t sms_rgb_parm3;
...
void render_line(int line)
{
    ...
    if (lcddev.width == 480) //480*800的屏幕,显示480*384大小
    {
        for (i = sx; i < ex; i++)
        {
            color = SMS_VDP->SMS_Palette[SMS_VDP->linebuf[i] & 0x1F]; //得到颜色值
            LCD->LCD_RAM = color;
            LCD->LCD_RAM = color;
            i++;
            color = SMS_VDP->SMS_Palette[SMS_VDP->linebuf[i] & 0x1F]; //得到颜色值
