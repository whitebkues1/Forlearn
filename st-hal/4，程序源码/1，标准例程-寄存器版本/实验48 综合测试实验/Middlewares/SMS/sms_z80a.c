...
int32_t CPU_exec(int32_t cycle, void *cpu);
...
int8_t Z80A_Init(uint8_t *SMS_Ram, uint8_t *Ext_Ram, uint8_t *Ext_Rom, uint8_t RomBanksMask)
{
    uint32_t i;//,k,p;
    //uint32_t* bb_p_tab;
    memset(Z80A, 0, sizeof(CPU80));		//清零Z80A所有内容
    Ram = SMS_Ram;
...
void CPU_Irq() //semplificata per solo RST38
{
    uint16_t ad;
...
int32_t CPU_run(int32_t cyc)
{
    return CPU_exec(cyc, &*Z80A);
}
...
/**
 * AC6编译时, 次函数会报错!!! 因为AC6嵌入汇编方式有变更, 此函数不支持  AC6编译!
 * 由于此函数改成支持AC6非常困难, 因此如果想用AC6, 建议不要支持SMS模拟器了...
 * 或者参考6502.s, 把整个sms_z80a.c改成汇编代码! 不过得用户自己努力了...
 */
//esegue codice Z80 per max cycle
int32_t __asm CPU_exec(int32_t cycle,void* cpu)
{
  PRESERVE8                    
