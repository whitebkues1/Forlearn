...
/* 12个主按钮的3中字符串表 */
uint8_t *const t9_mbtnstr_tbl[3][12] =
{
    {
        ";", "abc", "def", "←",
        "ghi", "jkl", "mno", "└┘",
...
/* 标点符号集 */
uint8_t *const t9_mbtnsign_tbl[T9_SIGNTBL_SIZE][9] =
{
    {
        ",", ".", "?",
        "!", ":", "、",
...
/* 5个纵向按钮 */
uint8_t *const t9_vbtnstr_tbl[5] = {"拼音", "123", "abc", "ABC", "手写"};

/**
 * @brief       创建输入法控件
 * @param       left,top        : 起始坐标
 * @param       width,height    : 宽度(必须为5的倍数)和高度(height-8,必须是6的倍数)
 * @param       mode            : 模式
 *                                [7:5]:0,保留
...
_t9_obj *t9_creat(uint16_t left, uint16_t top, uint16_t width, uint16_t height, uint8_t mode)
{
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        t9_crt->mbtn[i]->caption = (uint8_t *)t9_mbtnstr_tbl[0][i];
        t9_crt->mbtn[i]->bcfucolor = WHITE; /* 松开时为白色 */
        t9_crt->mbtn[i]->bcfdcolor = BLACK; /* 按下时为黑色 */
    }

    /* 8个文字按键 */
    tbtnw = width / 8 - 1;          /* 文字选择按钮的宽度 宽度 */
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        t9_crt->vbtn[i]->caption = (uint8_t *)t9_vbtnstr_tbl[i];
            ...
            else     /* 手写识别 */
            {
                ...
                if ((t9x->ncr_top + 1) < key->y && key->y < (t9x->ncr_top + t9x->ncr_height - 2) && (t9x->ncr_left + 1) < key->x && key->x < (t9x->ncr_left + t9x->ncr_width - 1))   /* 在手写框内部 */
                {
                    ...
                    if (t9x->ncr_tcnt < 200)   /* 记录最多200个坐标轨迹 */
                    {
                        ...
                        if ((t9x->pbuf[tcnt].x != (key->x - t9x->ncr_left)) || (t9x->pbuf[tcnt].y != (key->y - t9x->ncr_top)))   /* 新坐标,必须和老坐标不相等,否则不予采集!!! */
                        {
...
/**
 * @brief       更新横向8个按钮的caption
 * @param       t9x             : T9输入法指针
 * @param       str             : 按钮caption摘取的字符串.
 * @retval      无
 */
 ...
 * @brief       切换输入法
 * @param       t9x             : T9输入法指针
 * @param       type            : 0~4;0,拼音;1,123;2,abc;3,ABC,4,手写.
 * @retval      无
 */
void t9_set_inputtype(_t9_obj *t9x, uint8_t type)
{
    ...
    for (i = 0; i < 5; i++)
    {
        t9x->vbtn[i]->caption = (uint8_t *)t9_vbtnstr_tbl[i];
    ...
    if (type != 4)   /* 不是手写识别模式 */
    {
        ...
        for (i = 0; i < 12; i++)
        {
            t9x->mbtn[i]->caption = (uint8_t *)t9_mbtnstr_tbl[mx][i]; /* 12个主按钮的名字 */
 ...
 * @brief       清除按钮的caption
 * @param       t9x             : T9输入法指针
 * @param       btnx            : [2],0,不清除vbtn;1,清除vbtn;
...
void t9_load_sign(_t9_obj *t9x)
{
    ...
    for (i = 0; i < 9; i++)   /* 更新caption */
    {
        ...
        t9x->mbtn[i + offset]->caption = (uint8_t *)t9_mbtnsign_tbl[t9x->signpage][i];
    }
...
void t9_hbtn_process(_t9_obj *t9x, uint8_t id)
{
    ...
    t9_hbtn_updc(t9x, t9x->disstr + ((uint16_t)t9x->dispagecur * 12)); /* 更新按钮caption */
    t9_draw_hbtn(t9x);/* 重画横向按钮 */
}
...
void t9_vbtn_process(_t9_obj *t9x, uint8_t id)
{
    ...
    if (id < 24 && id > 20)   /* 在中间 */
    {
        ...
        {
            t9x->disstr = (uint8_t *)t9x->pymb[(t9x->pypage) * 3 + id - 21]->pymb; /* 选择新的拼音 */
        }
        else return;
    }
    else if (id == 20 || id == 24)     /* 上/下一页 */
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            t9x->vbtn[i + 1]->caption = (uint8_t *)t9x->pymb[temp + i]->py;
            t9x->vbtn[i + 1]->bcfucolor = 0XFFFF; /* 白色 */

            if (i == 0)t9x->disstr = (uint8_t *)t9x->pymb[temp + i]->pymb;  /* 第一个码表赋值给disstr */
        }
    }
