...
#define AUDIO_PLAY_STK_SIZE     800     /* 设置任务堆栈大小 */
...
void audio_play_task(void *pdata)
{
    ...
    while (audiodev.status & 0x80)
    {
        ...
        while (rval == 0 && audioinfo)  /* audioinfo必须非0 */
        {
            ...
            audiodev.name = (uint8_t *)(audioinfo->fname);
            ...
            if ((exfuns_file_type((char *)pname) & 0X40) == 0X40)       /* 是音频文件 */
            {
...
{
    uint8_t aui_tpbar_height = 0;   /* 顶部标题栏高度 */
    uint8_t aui_msgbar_height = 0;  /* 信息栏高度 */
    uint8_t aui_prgbar_height = 0;  /* 进度条区域高度 */
    uint8_t aui_btnbar_height = 0;  /* 底部按钮条高度 */
    uint8_t pixpitch = 0;
    uint8_t toffy = 0;
    uint8_t idx = 0;
    ...
    gui_show_strmid(0, 0, lcddev.width, aui_tpbar_height, AUDIO_TITLE_COLOR, 16, (uint8_t *)APP_MFUNS_CAPTION_TBL[2][gui_phy.language]);    /* 显示标题 */
    ...
    minibmp_decode((uint8_t *)APP_VOL_PIC, pixpitch + 2, aui_tpbar_height + 20 + toffy - 2, 16, 16, 0, 0);          /* 解码音量图标 */
    ...
    piclib_ai_load_picfile((char*)AUDIO_BACK_PIC[idx], 0, aui_tpbar_height + aui_msgbar_height, lcddev.width, lcddev.height - aui_tpbar_height - aui_msgbar_height - aui_prgbar_height - aui_btnbar_height, 0); /* 加载背景图片 */
...
void audio_info_upd(__audiodev *audiodevx, _progressbar_obj *audioprgbx, _progressbar_obj *volprgbx, _lyric_obj *lrcx)
{
    ...
    uint8_t aui_tpbar_height;   /* 顶部标题栏高度 */
    uint8_t aui_msgbar_height;  /* 信息栏高度 */
    uint8_t pixpitch;
    uint8_t toffy;
    uint16_t tempx;
    uint8_t *buf;
...
void audio_lrc_show(__audiodev *audiodevx, _lyric_obj *lrcx)
{
    uint8_t t;
    uint16_t temp, temp1;
    uint16_t lrcwidth = lcddev.width - 40;
    uint8_t lrcdheight;
    uint16_t sy;
    uint16_t syadd;
