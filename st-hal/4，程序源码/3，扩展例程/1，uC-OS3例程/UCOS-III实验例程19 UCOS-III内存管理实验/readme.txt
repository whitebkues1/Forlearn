 ...
 ***************************************************************************************************
 * 实验简介
 * 实验名称：uC/OS-III 内存管理实验
 * 实验平台：正点原子 精英F103开发板
 * 实验目的：学习 uC/OS-III 的内存管理
 ...
 * 2 LCD上显示总的内存块数量，并实时更新内存块的剩余数量。
 * 3 当KEY0按键被按下时，申请内存，并将申请到内存的首地址以字符串形式存入申请到
     的内存中，然后在LCD上显示申请到内存的内容；当KEY1按键被按下时，释放内存。
